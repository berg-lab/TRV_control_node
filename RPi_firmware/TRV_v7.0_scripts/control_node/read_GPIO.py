...
def save_setpoint(_s, cmd):
    ...
    try:
        ...
        if cmd == 'start':
            file.write("i:%s,y:%s,u:0,w:%s" % (node_ID[0], str(_s), str(pwm)))  # start with no manual override
        elif cmd == 'button_press':
            file.write("i:%s,y:%s,u:1,w:%s" % (node_ID[0], str(_s), str(pwm)))
        elif cmd == 'save_data':
            file.write("i:%s,y:%s,u:%s,w:%s" % (node_ID[0], str(_s), str(u), str(pwm)))     # save existing override status as well as current PWM value
        file.close()
    except:
        pass
...
old_time_60 = time.time()  # keep track of 60 second timer
...
while True:
    ...
    if GPIO.input(button1) == 0 and GPIO.input(button2) != 0:
        if config[0]['acf']['control_strategy'] == 'pid_temp' or config[0]['acf']['control_strategy'] == 'pid_temp_motion':
            ...
            save_setpoint(temp_setpoint, 'button_press')
        else:
            num_setpoint -= 1
            setpoint_display(num_setpoint)
            save_setpoint(num_setpoint, 'button_press')
